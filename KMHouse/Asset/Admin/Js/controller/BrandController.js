var brandconfig = {
    pageSize: $('#sltPageSize').val(),
    pageIndex: 1,
}
var brandController = {
    init: function () {
        brandController.loadData();
        brandController.registerEvent();
    },
    registerEvent: function () {
        $('#frmBrand').validate({
            rules: {
                txtName: {
                    required: true,
                },
            },
            messages: {
                txtName: {
                    required: "Bạn phải nhập tên",
                },
            }
        });
        $('#txtKeyword').off('keyup').on('keyup', function (e) {
            brandController.loadData(true);
        });
        $('#frmBrand').off('keypress').on('keypress', function (e) {
            if (e.which == 13) {
                if ($('#frmBrand').valid()) {
                    brandController.saveData();
                }
            }
        });
        $('#btnAddNew').off('click').on('click', function () {
            $('#modalAddUpdate').modal('show');
            brandController.resetForm();
        });

        $('#btnSave').off('click').on('click', function () {
            if ($('#frmBrand').valid()) {
                brandController.saveData();
            }
        });
        $('#sltPageSize').off('change').on('change', function () {
            brandconfig.pageSize = $(this).val();
            brandController.loadData(true);
        })
        $('#btnSearch').off('click').on('click', function () {
            brandController.loadData(true);
        });
        $('#btnReset').off('click').on('click', function () {
            $('#txtKeyword').val('');
            $('#sltTypeSearch').val(0);
            $('#sltPageSize').val(5);
            brandController.loadData(true);
        });
        $('.btn-edit').off('click').on('click', function () {
            $('#modalAddUpdate').modal('show');
            var id = $(this).data('id');
            brandController.loadDetail(id);
        });

        $('.btn-delete').off('click').on('click', function () {
            var id = $(this).data('id');
            bootbox.confirm("Bạn có muốn xóa không?", function (result) {
                if (result) {
                    brandController.deleteBrand(id);
                }
            });
        });
        $('#btnImage').off('click').on('click', function (e) {
            e.preventDefault();
            var finder = new CKFinder();
            finder.selectActionFunction = function (url) {
                $('#txtImage').val(url);
                $('#imgSlide').attr('src', url);
            };
            finder.popup();
        });
    },
    deleteBrand: function (id) {
        $.ajax({
            url: '/Brand/Delete',
            data: {
                id: id
            },
            type: 'POST',
            dataType: 'json',
            success: function (response) {
                if (response.status == 1) {
                    brandController.loadData(true);
                    $('#status').text('Deleted');
                    window.setTimeout(function () { $('#status').text('Normal'); }, 2000);
                }
                else if (response.status == 0) {
                    bootbox.alert("Có lỗi xảy ra. Vui lòng thử lại");
                }
                else
                    bootbox.alert("Có Sản phẩm đang dùng. Vui lòng thử lại");
            },
            error: function (err) {
                console.log(err);
            }
        });
    },
    loadDetail: function (id) {
        $.ajax({
            url: '/Brand/GetDetail',
            data: {
                id: id
            },
            type: 'GET',
            dataType: 'json',
            success: function (response) {
                if (response.status == true) {
                    var data = response.data;
                    $('#hidID').val(data.ID);
                    $('#txtName').val(data.Name);
                    $('#txtImage').val(data.Logo);
                    $('#imgSlide').attr('src', data.Logo);
                }
                else {
                    bootbox.alert(response.message);
                }
            },
            error: function (err) {
                console.log(err);
            }
        });
    },
    saveData: function () {
        var name = $('#txtName').val();
        var id = parseInt($('#hidID').val());
        var image = $('#txtImage').val();
        var brand = {
            Name: name,
            ID: id,
            Logo: image
        }
        $.ajax({
            url: '/Brand/SaveData',
            data: {
                strBrand: JSON.stringify(brand)
            },
            type: 'POST',
            dataType: 'json',
            success: function (response) {
                if (response.status == true) {
                    $('#modalAddUpdate').modal('hide');
                    brandController.loadData(true);
                    if (response.action == 'insert') {
                        $('#status').text('Add new success');
                        window.setTimeout(function () { $('#status').text('Normal'); }, 2000);
                    }
                    else {
                        $('#status').text('Updated');
                        window.setTimeout(function () { $('#status').text('Normal'); }, 2000);
                    }
                }
                else {
                    bootbox.alert(response.message);
                }
            },
            error: function (err) {
                console.log(err);
            }
        });
    },
    resetForm: function () {
        $('#hidID').val('0');
        $('#txtName').val('');
        $('#txtImage').val('');
        $('#imgSlide').attr('src', '');
    },
    loadData: function (changePageSize) {
        var keyword = $('#txtKeyword').val();
        var type = $('#sltTypeSearch').val();

        $.ajax({
            url: '/Brand/LoadData',
            type: 'GET',
            data: {
                keyword: keyword,
                type: type,
                pageIndex: brandconfig.pageIndex,
                pageSize: brandconfig.pageSize
            },
            dataType: 'json',
            success: function (response) {
                if (response.status) {
                    var data = response.data;
                    var html = '';
                    var template = $('#data-template').html();
                    $.each(data, function (i, item) {
                        html += Mustache.render(template, {
                            ID: item.ID,
                            Name: item.Name,
                            Logo: item.Logo
                        });
                    });
                    $('#tblData').html(html);
                    $('#lbTotal').text(response.totalCurent + " / " + response.total + ' tổng số bản ghi');
                    brandController.paging(response.total, function () {
                        brandController.loadData();
                    }, changePageSize);
                    brandController.registerEvent();
                }
            }
        })
    },
    paging: function (totalRow, callback, changePageSize) {
        var totalPage = Math.ceil(totalRow / brandconfig.pageSize);

        //Unbind pagination if it existed or click change pagesize
        if ($('#pagination a').length === 0 || changePageSize === true) {
            $('#pagination').empty();
            $('#pagination').removeData("twbs-pagination");
            $('#pagination').unbind("page");
        }

        $('#pagination').twbsPagination({
            totalPages: totalPage,
            first: "Đầu",
            next: "Tiếp",
            last: "Cuối",
            prev: "Trước",
            visiblePages: 10,
            onPageClick: function (event, page) {
                brandconfig.pageIndex = page;
                setTimeout(callback, 200);
            }
        });
    }
}
brandController.init();